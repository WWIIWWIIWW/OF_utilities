...
Description
    ...
    formula. Requires the mixtureFractionDict by kai.zhang.1@city.ac.uk
...
int main(int argc, char *argv[])
{
    ...
    Info << "Instantiate Chemistry Reader!" << endl;
    autoPtr<chemistryReader<gasHThermoPhysics>> fcr
    (
	chemistryReader<gasHThermoPhysics>::New(thermo, species)
    );

    const HashPtrTable<gasHThermoPhysics>& speciesThermo = fcr().speciesThermo(); 
    const speciesCompositionTable& specieComposition = fcr().specieComposition();
